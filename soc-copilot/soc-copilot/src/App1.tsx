...
interface ConfidenceFactor {
  ...
  contributes_to: 'Increase' | 'Decrease';
}
...
interface SiemAlert {
  ...
  playbook?: Playbook;
  threatIntelMatches?: ThreatIntelSource[];
}
...
interface FeedStatus {
  [key: string]: 'connected' | 'error' | 'connecting' | 'disconnected';
}

type ConnectionStatus = 'connected' | 'connecting' | 'error' | 'disconnected';
type Dataset = 'CICIDS2017' | 'UNSW-NB15' | 'CTU-13';
...
const SOCCopilot: React.FC = () => {
  ...
  const [simulatedDataset, setSimulatedDataset] = useState<Dataset>('CICIDS2017');
  const [siemAlerts, setSiemAlerts] = useState<SiemAlert[]>([]);

  // Generate realistic alerts based on selected dataset
  const generateAlertsFromDataset = (dataset: Dataset): SiemAlert[] => {
    const baseAlerts: Record<Dataset, SiemAlert[]> = {
      'CICIDS2017': [
        {
          id: "ALR-2024-001",
          ...
          confidence: {
            score: 85,
            factors: [
              { factor: "Known C2 IP address", weight: "High", contributes_to: "Increase" },
              { factor: "Encrypted communication", weight: "Medium", contributes_to: "Increase" },
              { factor: "Non-standard port usage", weight: "Medium", contributes_to: "Increase" }
            ],
            reasoning: "High confidence due to known malicious infrastructure."
          }
        },
        {
          id: "ALR-2024-002", 
          ...
          confidence: {
            score: 70,
            factors: [
              { factor: "Multiple suspicious domains", weight: "High", contributes_to: "Increase" },
              { factor: "Short time window", weight: "Medium", contributes_to: "Increase" }
            ],
            reasoning: "Moderate confidence based on domain reputation."
          }
        },
        {
          id: "ALR-2024-003",
          ...
          confidence: {
            score: 95,
            factors: [
              { factor: "Known malware hash", weight: "High", contributes_to: "Increase" },
              { factor: "Email delivery method", weight: "High", contributes_to: "Increase" }
            ],
            reasoning: "Very high confidence due to known malware signature."
          }
        }
      ],
      'UNSW-NB15': [
        {
          ...
          confidence: {
            score: 80,
            factors: [
              { factor: "Systematic scanning pattern", weight: "High", contributes_to: "Increase" },
              { factor: "Multiple target ports", weight: "Medium", contributes_to: "Increase" }
            ],
            reasoning: "High confidence based on scanning behavior."
          }
        }
      ],
      'CTU-13': [
        {
          ...
          confidence: {
            score: 75,
            factors: [
              { factor: "Periodic beaconing pattern", weight: "High", contributes_to: "Increase" },
              { factor: "Known botnet signatures", weight: "Medium", contributes_to: "Increase" }
            ],
            reasoning: "Good confidence based on traffic patterns."
          }
        }
      ]
    };
  ...
  const fetchRealThreatIntel = async (): Promise<void> => {
    setIsLoading(true);
    ...
    try {
      ...
      try {
        ...
        if (urlhausResponse.ok) {
          ...
          sources.push({
            ...
            sample_data: urlhausData.urls?.slice(0, 3).map((u: any) => u.url) || [],
            source: 'abuse.ch',
            last_updated: new Date().toISOString(),
            status: 'active'
          });
      ...
      try {
        ...
        if (malwareResponse.ok) {
          ...
          sources.push({
            ...
            sample_data: malwareData.data?.slice(0, 3).map((m: any) => m.sha256_hash) || [],
            source: 'abuse.ch',
            last_updated: new Date().toISOString(),
            status: 'active'
          });
      ...
      const simulatedSources: ThreatIntelSource[] = [
        {
          name: 'AlienVault OTX',
          type: 'mixed_iocs',
          count: 1247,
      ...
      const enrichedAlerts = currentAlerts.map(alert => {
        ...
        return {
          ...
          confidence: {
            ...alert.confidence!,
            score: matchedSources.length > 0 ? Math.min(95, alert.confidence!.score + 15) : alert.confidence!.score
          }
        };
      });
  ...
  const getSourceIcon = (type: string): JSX.Element => {
    switch(type) {
  ...
  const handleDatasetChange = (e: React.ChangeEvent<HTMLSelectElement>): void => {
    setSimulatedDataset(e.target.value as Dataset);
  };
        ...
        <div className="lg:col-span-2">
          {selectedAlert ? (
            <div className="space-y-6">
              ...
              {/* Confidence Analysis */}
              <div className="bg-white rounded-lg shadow-md">
                <div className="p-4 border-b cursor-pointer" onClick={() => toggleSection('confidence')}>
                  <div className="flex justify-between items-center">
                    <h3 className="text-lg font-semibold flex items-center gap-2">
                      <BarChart3 className="h-5 w-5 text-green-600" />
                      Confidence Analysis
                    </h3>
                    {expandedSections.confidence ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                  </div>
                </div>
                  ...
                  <div className="p-4">
                    <div className="mb-4">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className={`text-2xl font-bold ${getConfidenceColor(selectedAlert.confidence?.score || 75)}`}>
                          {selectedAlert.confidence?.score || 75}%
                        </span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          ...
                          style={{width: `${selectedAlert.confidence?.score || 75}%`}}
                        ></div>
                      </div>
                    </div>
                    ...
                    {selectedAlert.confidence && (
                      <div className="mb-4">
                        ...
                        <div className="space-y-2">
                          {selectedAlert.confidence.factors.map((factor, index) => (
                            <div key={index} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                              <span className="text-sm">{factor.factor}</span>
                              <div className="flex items-center gap-2">
                                <span className={`text-xs px-2 py-1 rounded ${
                                  factor.weight === 'High' ? 'bg-red-100 text-red-700' :
                                  factor.weight === 'Medium' ? 'bg-orange-100 text-orange-700' :
                                  'bg-yellow-100 text-yellow-700'
                                }`}>
                                  {factor.weight}
                                </span>
                                <span className={`text-xs px-2 py-1 rounded ${
                                  factor.contributes_to === 'Increase' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                                }`}>
                                  {factor.contributes_to === 'Increase' ? '↑' : '↓'}
                                </span>
                              </div>
                            </div>
                          ))}
                    ...
                    {selectedAlert.confidence && (
                      <div>
                        ...
                        <p className="text-gray-700 text-sm italic">{selectedAlert.confidence.reasoning}</p>
                      </div>
                    )}
                  </div>
                )}
